...
export const recordAttendanceByQr = createAsyncThunk(
  ...
  async ({ id_anak, id_aktivitas, status, token }, { rejectWithValue }) => {
    try {
      const response = await attendanceApi.recordAttendanceByQr(id_anak, id_aktivitas, status, token);
...
export const recordAttendanceManually = createAsyncThunk(
  ...
  async ({ id_anak, id_aktivitas, status, notes }, { rejectWithValue }) => {
    try {
      const response = await attendanceApi.recordAttendanceManually(id_anak, id_aktivitas, status, notes);
      return response.data;
    } catch (error) {
      // Special handling for duplicate records (409 Conflict)
      if (error.response?.status === 409) {
...
export const selectActivityAttendance = (state, id_aktivitas) => state.attendance.activityRecords[id_aktivitas] || [];
export const selectStudentAttendance = (state, id_anak) => state.attendance.studentRecords[id_anak] || [];
export const selectVerificationHistory = (state, id_absen) => state.attendance.verificationHistory[id_absen] || [];
