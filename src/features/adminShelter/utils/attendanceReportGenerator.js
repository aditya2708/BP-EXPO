...
class AttendanceReportGenerator {
    ...
    if (options.groupByStudent) {
      attendanceRecords.forEach(record => {
        if (record.absen_user && record.absen_user.anak) {
          ...
          studentStats[id].rate = (studentStats[id].present / studentStats[id].total) * 100;
        }
      });
    }
  ...
  static generatePdfReport(attendanceRecords, stats, options = {}) {
    ...
    const statsData = [
      ...
      ['Present', `${stats.present} (${stats.attendanceRate.toFixed(1)}%)`],
    ...
    if (stats.studentStats && stats.studentStats.length > 0) {
      ...
      doc.autoTable({
        startY: studentY + 4,
        head: [['Student', 'Total', 'Present', 'Absent', 'Rate']],
        body: studentData,
        theme: 'grid',
        headStyles: { fillColor: [41, 128, 185] }
      });
    ...
    if (options.includeRecords && attendanceRecords.length > 0) {
      ...
      if (recordsY > 260) {
        ...
        const recordsData = attendanceRecords.map(record => {
          ...
          const status = record.absen === 'Ya' ? 'Present' : 'Absent';
        ...
        doc.autoTable({
          startY: 24,
          head: [['Student', 'Status', 'Verified', 'Verification', 'Date']],
          body: recordsData,
          theme: 'grid',
          headStyles: { fillColor: [41, 128, 185] },
          styles: { fontSize: 9 },
  ...
  static generateCsvData(attendanceRecords, options = {}) {
    ...
    // Define headers
    let headers = ['Student ID', 'Student Name', 'Status', 'Verified', 'Verification Method', 'Activity', 'Date'];
