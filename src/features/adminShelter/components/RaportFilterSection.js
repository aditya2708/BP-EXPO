...
const RaportFilterSection = ({
  ...
  const handleMataPelajaranChange = (mataPelajaran) => {
    setShowMataPelajaranModal(false);
    setTempFilters(prev => ({
      ...prev,
      mata_pelajaran: prev.mata_pelajaran === mataPelajaran ? null : mataPelajaran
    }));
  };
  ...
  const handleApply = () => {
    const formattedFilters = {
      ...
      mata_pelajaran: tempFilters.mata_pelajaran
    };
  ...
  const hasActiveFilters = tempFilters.start_date || tempFilters.end_date || tempFilters.mata_pelajaran;

  const renderMataPelajaranModal = () => (
    <Modal
      visible={showMataPelajaranModal}
      transparent
      animationType="slide"
      onRequestClose={() => setShowMataPelajaranModal(false)}
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Pilih Mata Pelajaran</Text>
            <TouchableOpacity onPress={() => setShowMataPelajaranModal(false)}>
          ...
          <TouchableOpacity
            style={[
              styles.modalItem,
              !tempFilters.mata_pelajaran && styles.modalItemSelected
            ]}
            onPress={() => handleMataPelajaranChange(null)}
          >
            <Text style={[
              styles.modalItemText,
              !tempFilters.mata_pelajaran && styles.modalItemTextSelected
            ]}>
              Semua Mata Pelajaran
            </Text>
            {!tempFilters.mata_pelajaran && (
              <Ionicons name="checkmark" size={20} color="#9b59b6" />
            )}
          </TouchableOpacity>
          
          <FlatList
            data={filterOptions.availableMataPelajaran || []}
            keyExtractor={(item) => item}
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.modalItem,
                  tempFilters.mata_pelajaran === item && styles.modalItemSelected
                ]}
                onPress={() => handleMataPelajaranChange(item)}
              >
                <Text style={[
                  styles.modalItemText,
                  tempFilters.mata_pelajaran === item && styles.modalItemTextSelected
                ]}>
                  {item}
                </Text>
                {tempFilters.mata_pelajaran === item && (
                  <Ionicons name="checkmark" size={20} color="#9b59b6" />
                )}
              </TouchableOpacity>
            )}
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          <View style={styles.header}>
            <Text style={styles.title}>Filter Laporan Raport</Text>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.section}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setShowMataPelajaranModal(true)}
              >
                <View style={styles.filterButtonContent}>
                  <Text style={styles.filterLabel}>Mata Pelajaran</Text>
                  <Text style={[
                    styles.filterValue,
                    !tempFilters.mata_pelajaran && styles.filterValuePlaceholder
                  ]}>
                    {tempFilters.mata_pelajaran || 'Semua Mata Pelajaran'}
                  </Text>
                </View>
...
export default RaportFilterSection;