...
const AdminShelterDashboardScreen = () => {
  ...
  const navigateToChildrenManagement = () => navigation.navigate('ChildrenManagement');
    ...
    >
      ...
      <View style={styles.statsRow}>
        <View style={[styles.statCard, { backgroundColor: '#e74c3c' }]}>
          ...
          <Text style={styles.statNumber}>{dashboardData?.children_count || 0}</Text>
          <Text style={styles.statLabel}>Children</Text>
        </View>
        
        <View style={[styles.statCard, { backgroundColor: '#9b59b6' }]}>
          <Ionicons name="heart" size={24} color="#ffffff" />
          <Text style={styles.statNumber}>{dashboardData?.donatur_count || 0}</Text>
          <Text style={styles.statLabel}>Donatur</Text>
        </View>
        
        <View style={[styles.statCard, { backgroundColor: '#3498db' }]}>
          <Ionicons name="calendar" size={24} color="#ffffff" />
          <Text style={styles.statNumber}>{dashboardData?.attendance_count || 0}</Text>
      ...
      <View style={styles.sectionContainer}>
        ...
        <View style={styles.quickActionsContainer}>
          <TouchableOpacity 
            ...
            onPress={navigateToChildrenManagement}
          >
      ...
      <View style={styles.sectionContainer}>
        ...
        {dashboardData?.recent_children?.length > 0 ? (
          <ScrollView 
            horizontal 
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={styles.childrenScrollContent}
          >
            {dashboardData.recent_children.map((child, index) => (
              <TouchableOpacity 
                key={index} 
                style={styles.childCard}
                onPress={() => navigation.navigate('ChildDetail', { id: child.id_anak })}
              >
                {child.foto ? (
                  <Image
                ...
                <Text style={styles.childName} numberOfLines={1}>{child.nama_lengkap}</Text>
