...
const ActivityFormScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (isEditing && activity) {
      ...
      // Set custom threshold flag if late_threshold exists
  ...
  const toggleCustomLateThreshold = (value) => {
    ...
    if (!value) {
      // Reset to using minutes threshold
      handleChange('late_threshold', null);
    } else if (formData.start_time && !formData.late_threshold) {
      // Initialize late threshold to 15 minutes after start time
  ...
  const handleSubmit = async () => {
    ...
    // Validate late threshold consistency
