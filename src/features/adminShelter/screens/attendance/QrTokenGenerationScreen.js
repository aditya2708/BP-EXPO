...
const QrTokenGenerationScreen = ({ navigation, route }) => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      if (response.data && response.data.data) {
        // Get active students (status_validasi === 'aktif')
        const activeStudents = response.data.data.filter(
          student => student.status_validasi === 'aktif' || student.status_validasi === 'Aktif'
        );
  ...
  const renderStudentItem = ({ item }) => {
    ...
    const token = useSelector(state => selectStudentToken(state, item.id_anak));
