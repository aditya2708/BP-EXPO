...
const createEntityThunks = (entityType) => {
  ...
  thunks.fetchAll = createAsyncThunk(
    ...
    async (params = {}, { rejectWithValue }) => {
      // 🐛 DEBUG: Log thunk execution
      console.log(`🚀 Redux thunk ${entityType}/fetchAll started:`, {
        entityType,
      ...
      try {
        ...
        console.log(`✅ Redux thunk ${entityType}/fetchAll success:`, {
        ...
        console.error(`❌ Redux thunk ${entityType}/fetchAll failed:`, {
    ...
    extraReducers: (builder) => {
      ...
      builder
        .addCase(thunks.fetchAll.pending, (state) => {
          console.log(`⏳ ${entityType}/fetchAll.pending - Setting loading state`);
          state.listLoading = true;
          state.listError = null;
        })
        .addCase(thunks.fetchAll.fulfilled, (state, action) => {
          // Handle nested data structure: payload.data.data (API has pagination wrapper)
          const responseData = action.payload.data || action.payload;
          const actualItems = responseData.data || responseData;
          
          console.log(`✅ ${entityType}/fetchAll.fulfilled - Updating state:`, {
            fullPayload: action.payload,
            responseData: responseData,
            actualItems: actualItems,
            itemsLength: Array.isArray(actualItems) ? actualItems.length : 0,
            totalFromAPI: responseData.total
          });
          
          state.listLoading = false;
          // ✅ FIX: Handle nested data structure
          state.items = Array.isArray(actualItems) ? actualItems : [];
          state.totalItems = responseData.total || 0;
          state.currentPage = responseData.current_page || 1;
          state.totalPages = responseData.last_page || 1;
          state.lastFetch = Date.now();
        })
        .addCase(thunks.fetchAll.rejected, (state, action) => {
          console.error(`❌ ${entityType}/fetchAll.rejected - Error:`, action.payload);
          state.listLoading = false;
      ...
      builder
        ...
        .addCase(thunks.fetchById.fulfilled, (state, action) => {
          // Handle nested data structure consistently
          const responseData = action.payload.data || action.payload;
          console.log(`✅ ${entityType}/fetchById.fulfilled - Setting current item:`, responseData);
          state.itemLoading = false;
          state.currentItem = responseData;
        })
      ...
      builder
        ...
        .addCase(thunks.create.fulfilled, (state, action) => {
          ...
          console.log(`✅ ${entityType}/create.fulfilled - Adding new item:`, newItem);
          
          // Add to beginning of items array if it's an array
      ...
      builder
        ...
        .addCase(thunks.update.fulfilled, (state, action) => {
          ...
          console.log(`✅ ${entityType}/update.fulfilled - Updating item:`, updatedItem);
      ...
      builder
        ...
        .addCase(thunks.delete.fulfilled, (state, action) => {
          ...
          console.log(`✅ ${entityType}/delete.fulfilled - Removing item with ID:`, deletedId);
