...
const MateriAssignmentModal = ({
  ...
  // ✨ REFACTORED: Use unified hooks instead of individual selectors
  const {
    availableMateri,
  ...
  const {
    dropdownOptions: mataPelajaranOptions,
    loading: mataPelajaranLoading,
    getDropdownOptions: getMataPelajaranDropdown
  } = useEntityCRUD('mataPelajaran');

  const {
    dropdownOptions: kelasOptions,
    loading: kelasLoading,
    getDropdownOptions: getKelasDropdown
  } = useEntityCRUD('kelas');
  ...
  const loadAvailableMateri = async (pageNum = 1, reset = false) => {
    const params = {
      page: pageNum,
      per_page: 20,
    };
    ...
    try {
      await getAvailableMateri(kurikulumId, params);
      ...
      console.error('Error loading available materi:', err);
    }
  };
  ...
  const handleAssignMateri = async () => {
    ...
    try {
      ...
      await assignMateri(kurikulumId, materiIds);
    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerActions}>
        ...
        <Text style={styles.countText}>
          {availableMateri?.length || 0} materi tersedia
        </Text>
      </View>
    </View>
  );
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.listContainer}>
          {kurikulumLoading && (!availableMateri || availableMateri.length === 0) ? (
