...
const EntityCard = ({ item, config, onEdit, onDelete, onDetail }) => {
  ...
  const getSecondaryText = () => {
    switch (config.ui.title) {
      case 'Jenjang':
        return `${item.kode || ''} • ${item.deskripsi || 'Tidak ada deskripsi'}`;
      case 'Mata Pelajaran':
...
const EntityList = ({ 
  ...
  const handleDelete = useCallback((item) => {
    ...
    const itemId = item[config.api.idField] || item.id;
  ...
  }, [deleteItem, fetchItems, config.api.nameField, config.api.idField, showDeleteConfirmation]);
