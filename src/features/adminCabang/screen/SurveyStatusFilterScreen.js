...
const SurveyStatusFilterScreen = () => {
  ...
  const tabs = [
    { key: 'pending', label: 'Pending', icon: 'time-outline', color: '#f39c12' },
    { key: 'approved', label: 'Approved', icon: 'checkmark-circle-outline', color: '#27ae60' },
    { key: 'rejected', label: 'Rejected', icon: 'close-circle-outline', color: '#e74c3c' }
  ];
  ...
  const getStatusBadge = (status, approvedAt) => {
    const config = {
      pending: { label: 'PENDING', color: '#f39c12' },
      layak: { label: 'APPROVED', color: '#27ae60' },
      'tidak layak': { label: 'REJECTED', color: '#e74c3c' }
    };
  ...
  const renderSurveyItem = ({ item }) => (
    <TouchableOpacity style={styles.surveyCard} onPress={() => navigateToDetail(item)}>
      <View style={styles.surveyHeader}>
        ...
        {getStatusBadge(item.hasil_survey, item.approved_at)}
      </View>
      ...
      <View style={styles.surveyInfo}>
        ...
        {item.approved_at && (
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoText}>Processed: {new Date(item.approved_at).toLocaleDateString()}</Text>
          </View>
        )}
      </View>
      ...
      {activeTab !== 'pending' && item.approvedBy && (
        <View style={styles.processedInfo}>
          <Text style={styles.processedLabel}>
            Processed by: {item.approvedBy.nama_lengkap || 'Admin'}
          </Text>
        </View>
      )}
    </TouchableOpacity>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        <Text style={styles.statsText}>
          {pagination.total || 0} {tabs.find(t => t.key === activeTab)?.label} Surveys
        </Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>No {activeTab} surveys found</Text>
          </View>
        }
      />
    </View>
