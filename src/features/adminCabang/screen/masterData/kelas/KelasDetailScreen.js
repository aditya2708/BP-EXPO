// src/features/adminCabang/screens/masterData/kelas/KelasDetailScreen.js
...
import { fetchKelasById, deleteKelas } from '../../../redux/masterData/kelasSlice';
...
const KelasDetailScreen = ({ navigation, route }) => {
  ...
  const handleDelete = () => {
    Alert.alert(
      'Hapus Kelas',
      `Yakin hapus "${currentKelas?.nama}"? Data materi terkait akan terpengaruh.`,
      [
  ...
  if (!currentKelas) {
    return (
      <SafeAreaWrapper>
        <Text style={styles.errorText}>Kelas tidak ditemukan</Text>
      </SafeAreaWrapper>
    );
  }

  const stats = [
    { label: 'Total Materi', value: currentKelas.materi_count || 0, icon: 'document-text', color: '#e74c3c' },
    { label: 'Mata Pelajaran', value: currentKelas.mata_pelajaran_count || 0, icon: 'book', color: '#2ecc71' },
    { label: 'Kurikulum', value: currentKelas.kurikulum_count || 0, icon: 'library', color: '#8e44ad' }
  ];

  return (
    <SafeAreaWrapper>
      <DetailHeader
        title={currentKelas.nama}
        subtitle="Detail Kelas"
        onBack={() => navigation.goBack()}
        onEdit={() => navigation.navigate('KelasForm', { id })}
        error={error}
      />
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <DetailCard title="Informasi Kelas">
          <View style={styles.infoRow}>
            <Text style={styles.label}>Nama:</Text>
            <Text style={styles.value}>{currentKelas.nama}</Text>
          </View>
          
          <View style={styles.infoRow}>
            <Text style={styles.label}>Jenjang:</Text>
            <Text style={styles.value}>{currentKelas.jenjang?.nama || 'Tidak ada'}</Text>
          </View>
          
          <View style={styles.infoRow}>
            <Text style={styles.label}>Tingkat:</Text>
            <Text style={styles.value}>{currentKelas.tingkat}</Text>
          </View>
          
          {currentKelas.deskripsi && (
            <View style={styles.infoRow}>
              <Text style={styles.label}>Deskripsi:</Text>
              <Text style={styles.value}>{currentKelas.deskripsi}</Text>
            </View>
          )}
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.value}>
              {new Date(currentKelas.created_at).toLocaleDateString('id-ID')}
            </Text>
          </View>
        </DetailCard>

        {currentKelas.materi?.length > 0 && (
          <DetailCard title="Materi Terkait">
            {currentKelas.materi.map((materi) => (
              <TouchableOpacity 
                key={materi.id}
              ...
              >
                <View style={styles.relatedInfo}>
                  ...
                  <Text style={styles.relatedDetail}>{materi.mata_pelajaran?.nama || 'Tanpa mata pelajaran'}</Text>
                </View>
      ...
      <View style={styles.actionContainer}>
        <TouchableOpacity style={styles.editButton} onPress={() => navigation.navigate('KelasForm', { id })}>
