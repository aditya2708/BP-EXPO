      ...
      actions: {
            ...
            if (response.success) {
              ...
              data.forEach(item => {
                if (item.id_kurikulum) {
                  normalized[item.id_kurikulum] = item;
                }
              });
              
              set(state => ({
                kurikulum: { ...state.kurikulum, ...normalized }
              }), false, 'kurikulum/load');
              
              return { success: true, data };
            }
            
            throw new Error(response.message || 'Failed to load kurikulum');
          } catch (error) {
            throw error;
          }
        },
        ...
        createKurikulum: async (data) => {
          try {
            ...
            if (response.success) {
              const newKurikulum = response.data;
            ...
            throw new Error(response.message || 'Failed to create kurikulum');
          } catch (error) {
            throw error;
          }
        },
        ...
        updateKurikulum: async (id, data) => {
          try {
            ...
            if (response.success) {
              const updatedKurikulum = response.data;
              ...
              return { success: true, data: updatedKurikulum };
            }
            
            throw new Error(response.message || 'Failed to update kurikulum');
          } catch (error) {
            throw error;
          }
        },
        ...
        deleteKurikulum: async (id) => {
          try {
            ...
            const response = await kurikulumApi.delete(id);
            ...
            throw new Error(response.message || 'Failed to delete kurikulum');
          } catch (error) {
            throw error;
          }
        },
        ...
        assignMateri: async (kurikulumId, materiIds, assignmentData = {}) => {
          try {
            ...
            if (response.success) {
              ...
              }), false, `kurikulum/assignMateri/${kurikulumId}`);
              
              return { success: true, data: Object.values(assignments) };
            }
            
            throw new Error(response.message || 'Failed to assign materi');
          } catch (error) {
            throw error;
          }
        },
        ...
        removeMateriAssignment: async (kurikulumId, materiId) => {
          try {
            ...
            throw new Error(response.message || 'Failed to remove materi assignment');
          } catch (error) {
            throw error;
          }
        },
        ...
        bulkAssignMateri: async (kurikulumId, materiIds, assignmentData = {}) => {
          try {
            return await get().actions.assignMateri(kurikulumId, materiIds, assignmentData);
          } catch (error) {
            throw error;
          }
        },
        
        // Reorder materi assignments
        reorderMateri: async (kurikulumId, materiOrders) => {
          try {
            const response = await kurikulumApi.reorderMateri(kurikulumId, materiOrders);
            
            if (response.success) {
              // Update local assignments order
              set(state => {
                const newAssignments = { ...state.kurikulumMateri };
                
                materiOrders.forEach(order => {
                  const assignmentId = `${kurikulumId}_${order.id_materi}`;
                  if (newAssignments[assignmentId]) {
                    newAssignments[assignmentId].urutan = order.urutan;
                  }
                });
                
                return { kurikulumMateri: newAssignments };
              }, false, `kurikulum/reorder/${kurikulumId}`);
              
              return { success: true };
            }
            
            throw new Error(response.message || 'Failed to reorder materi');
          } catch (error) {
            throw error;
          }
        },
        ...
        getAvailableMateri: async (kurikulumId, filters = {}) => {
          try {
            const response = await kurikulumApi.getAvailableMateri(kurikulumId, filters);
            
            if (response.success) {
              return { success: true, data: response.data };
            }
            
            throw new Error(response.message || 'Failed to get available materi');
          } catch (error) {
            throw error;
          }
        },
        ...
        changeStatus: async (kurikulumId, newStatus) => {
          try {
            ...
            // If activating, validate completeness and use setActive API
            if (newStatus === KURIKULUM_STATUS.ACTIVE) {
              ...
              const response = await kurikulumApi.setActive(kurikulumId);
              
              if (response.success) {
                // Update all kurikulum statuses based on API response
                set(state => {
                  const newKurikulum = { ...state.kurikulum };
                  
                  // Deactivate all others of same type
                  Object.keys(newKurikulum).forEach(id => {
                    if (newKurikulum[id].type === kurikulum.type) {
                      newKurikulum[id].status = id === kurikulumId ? KURIKULUM_STATUS.ACTIVE : KURIKULUM_STATUS.ARCHIVED;
                      newKurikulum[id].status_changed_at = new Date().toISOString();
                    }
                  });
                  
                  return { kurikulum: newKurikulum };
                }, false, `kurikulum/setActive/${kurikulumId}`);
                
                return { success: true };
              }
              
              throw new Error(response.message || 'Failed to activate kurikulum');
            } else {
              // For other status changes, use regular update
              return await get().actions.updateKurikulum(kurikulumId, { 
                status: newStatus,
        ...
        loadTemplates: async () => {
          try {
            // For now, use predefined templates since no specific API endpoint
            const defaultTemplates = {
              ...
              'template_sma': {
                id: 'template_sma',
                name: 'Template Kurikulum SMA',
                description: 'Template standar untuk Sekolah Menengah Atas',
                type: KURIKULUM_TYPES.NASIONAL,
                jenjang: ['SMA'],
                materiAssignments: []
              }
        ...
        getStatistics: async () => {
          try {
            const response = await kurikulumApi.getStatistics();
            ...
            if (response.success) {
              ...
              }), false, 'kurikulum/loadStatistics');
