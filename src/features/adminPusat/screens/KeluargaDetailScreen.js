...
const KeluargaDetailScreen = () => {
  ...
  const fetchKeluargaDetails = async () => {
    try {
      setError(null);
      const response = await adminPusatKeluargaApi.getKeluargaDetail(id);
      
      if (response.data.success) {
        setKeluargaData(response.data.data);
        // Set screen title based on family head name
        navigation.setOptions({ 
          headerTitle: response.data.data.keluarga?.kepala_keluarga || 'Detail Keluarga' 
        });
      } else {
        setError(response.data.message || 'Gagal memuat detail keluarga');
      }
    } catch (err) {
  ...
  const handleDeleteKeluarga = () => {
    Alert.alert(
      'Hapus Keluarga',
      `Apakah Anda yakin ingin menghapus keluarga "${keluargaData.keluarga.kepala_keluarga}"? Ini juga akan menghapus semua data terkait termasuk anak.`,
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              const response = await adminPusatKeluargaApi.deleteKeluarga(id);
              
              if (response.data.success) {
                Alert.alert(
                  'Sukses',
                  'Keluarga telah dihapus',
                  [
                    {
                      text: 'OK',
                ...
                setError(response.data.message || 'Gagal menghapus keluarga');
                setLoading(false);
              }
            } catch (err) {
              console.error('Error deleting keluarga:', err);
              setError('Gagal menghapus keluarga. Silakan coba lagi.');
              setLoading(false);
            }
          }
        }
  ...
  if (loading && !refreshing) {
    return <LoadingSpinner fullScreen message="Memuat detail keluarga..." />;
  }
                    ...
                    <View style={styles.childImageContainer}>
                      {child.foto_url ? (
                        ...
                        <View style={styles.childImagePlaceholder}>
                          <Ionicons name={child.jenis_kelamin === 'Laki-laki' ? 'Laki-laki' : 'Perempuan'} size={24} color="#fff" />
                        </View>
                      )}
                    </View>
                    ...
                    <View style={styles.childInfo}>
                      ...
                      <View style={[
                        ...
                        <Text style={styles.statusText}>
                          {child.status_validasi === 'aktif' ? 'Aktif' : 'Non-aktif'}
                        </Text>
                      </View>
                    </View>
