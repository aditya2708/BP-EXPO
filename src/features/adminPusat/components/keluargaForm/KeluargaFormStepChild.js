...
const KeluargaFormStepChild = ({
  ...
  const { getFieldStatus, createFieldRef } = useFormValidation(validationErrors);

  // Create refs for key form fields
  const nikAnakRef = createFieldRef('nik_anak');
  const anakKeRef = createFieldRef('anak_ke');
  const dariBersaudaraRef = createFieldRef('dari_bersaudara');
  const nickNameRef = createFieldRef('nick_name');
  const fullNameRef = createFieldRef('full_name');
  ...
  const validateStepFields = () => {
    ...
    // NIK validation
    if (formData.nik_anak && formData.nik_anak.length !== 16) {
      errors.nik_anak = 'NIK harus 16 digit';
    }

    // Required fields validation
    if (!formData.anak_ke) errors.anak_ke = 'Anak ke wajib diisi';
    if (!formData.dari_bersaudara) errors.dari_bersaudara = 'Jumlah bersaudara wajib diisi';
    if (!formData.nick_name) errors.nick_name = 'Nama panggilan wajib diisi';
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        fieldStatus={getFieldStatus('nik_anak', formData.nik_anak)}
      />
      ...
      <View style={styles.rowContainer}>
        <View style={[styles.inputContainer, styles.inputHalf]}>
          ...
          <TextInput
            ...
            fieldStatus={getFieldStatus('anak_ke', formData.anak_ke)}
          />
        </View>
        ...
        <View style={[styles.inputContainer, styles.inputHalf]}>
          ...
          <TextInput
            ...
            inputProps={{ keyboardType: 'numeric' }}
            fieldStatus={getFieldStatus('dari_bersaudara', formData.dari_bersaudara)}
          />
        </View>
      </View>
      ...
      <TextInput
        ...
        fieldStatus={getFieldStatus('nick_name', formData.nick_name)}
      />
      ...
      <TextInput
        ...
        placeholder=""
        leftIcon={<Ionicons name="person-outline" size={20} color="#777" />}
        fieldStatus={getFieldStatus('full_name', formData.full_name)}
      />
